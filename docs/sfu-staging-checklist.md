# Mediasoup SFU staging checklist

Manual verification after deploys that touch **`RiffSyncSfu`**, **`RiffSyncApi`** (`sfu-token`), or fan SPA env (**`VITE_PUBLIC_SFU_WS_URL`**, **`VITE_WEBRTC_USE_MEDIASOU_SFU`**).
