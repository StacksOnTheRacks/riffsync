...
The **canonical store** keeps operator-owned playback fields (**`youtubeVideoId`**, titles, catalog taxonomy, ids) and optional operator hints in Dynamo (**`movieSearchTitle`**, **`embedAllows`**, …) that are **not** required columns in git. **`data/catalog/episodes.json`** is a bootstrap/export snapshot constrained by **`catalog.schema.json`** — slim rows expose **`catalog`**, **`tags`**, **`labels`**, **`tagline`**, **`posterImageUrl`**, **`backdropImageUrl`**, **`tmdbMovieId`**, **`tmdbArtworkSyncedAt`** (often **`null`** until reconcile) alongside YouTube identifiers. Dynamo and **`GET /v1/catalog`** may still persist and return broader TMDB-derived copy (below) independent of what you commit as JSON seeds.
...
- **Canonical (DynamoDB):** system of record for `youtubeVideoId`, display **`title`**, **`catalog`**, **`tags`**, **`labels`**, stable **`id`**, optional operator hints (**`movieSearchTitle`**, **`tmdbMovieId`**, **`embedAllows`**). During early development this may be seeded from **`data/catalog/episodes.json`**; afterward treat JSON as fixtures / exports only.
