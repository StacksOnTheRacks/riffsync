...
| **Smoke band** | After **`deploy-prod.yml`** phase 5 when M27–M29 (and M33 subcategory prerender) are live, run **`npm run smoke:production`** (**[`scripts/launch-readiness/smoke-production.mjs`](../../scripts/launch-readiness/smoke-production.mjs)**). Asserts apex reachability, **`www`** → apex **301**, **`robots.txt`**/**`sitemap.xml`** **200**, apex canonical **`<link>`** on **`/`**, fixture **`/watch/101-the-crawling-eye`**, and **`/catalog/mst3k`**, and no **`www.riffsync.tv`** absolute URLs in shipped **`index.html`**. **Not** CI-wired. |
