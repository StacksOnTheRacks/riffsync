...
| Check | Contract |
| --- | --- |
| **Canonical hostname redirect and clean URL serving** | GitHub Actions repository variables set **`PROD_FAN_WEB_HOSTNAME=riffsync.tv`**, **`PROD_FAN_WEB_ALTERNATE_DOMAIN_NAMES=www.riffsync.tv`**, **`PROD_FAN_WEB_CANONICAL_HOSTNAME=riffsync.tv`** (see **`infra/cdk/README.md`**). **`fanWebCanonicalHostname`** CDK context on **`RiffSyncStatic-prod`** (**[`infra/cdk/lib/static-site-stack.ts`](../../infra/cdk/lib/static-site-stack.ts)**) is apex **`riffsync.tv`**; **`www.riffsync.tv`** is a **`fanWebAlternateDomainNames`** entry that **[`cloudfront-canonical-redirect.ts`](../../infra/cdk/lib/cloudfront-canonical-redirect.ts)** **301**-redirects to apex, preserving path and query. The same viewer-request CloudFront Function rewrites clean canonical URLs such as **`/catalog`** and **`/watch/{id}`** to their prerendered **`index.html`** S3 object keys before origin fetch. If apex redirects to **`www`** or clean indexable URLs return **`spa-shell.html`** / **`noindex`**, production SEO is not ready. |
...
| **Smoke band** | After **`deploy-prod.yml`** phase 5 when M27–M29 (and M33 subcategory prerender) are live, run **`npm run smoke:production`** (**[`scripts/launch-readiness/smoke-production.mjs`](../../scripts/launch-readiness/smoke-production.mjs)**). Asserts apex reachability, **`www`** → apex **301**, **`robots.txt`**/**`sitemap.xml`** **200**, clean URL prerender head tags (home, **`/catalog`**, **`/catalog/mst3k`**, fixture **`/watch/101-the-crawling-eye`**), no **`noindex`** on indexable clean URLs, and no **`www.riffsync.tv`** absolute URLs in shipped **`index.html`**. **Not** CI-wired. |
...
- No open decisions remain for M33 (#341). Normative smoke includes clean URL **`/catalog/mst3k`** 200 + MST3K prerender title + apex canonical + no noindex; packaging mechanics and head-tag ownership: **`build_packaging.md`** → *Decisions (M33 — catalog subcategory SEO packaging — #341)*.
