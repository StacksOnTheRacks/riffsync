...
| **Custom playback iframes** | Solo watch and party-capture source tabs may embed **arbitrary HTTPS origins** staff curate (**no domain allowlist** at catalog validation). CSP must permit framing those origins without enumerating partner domains. The room host shell does not embed Custom directly. |
