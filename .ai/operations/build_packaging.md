...
| **`sitemap.xml`** | Same build step; enumerates the static indexable routes plus one **`<url>`** entry per catalog episode passing **`episodeIsIndexableForSeo`** (**#397**) | **`data/catalog/episodes.json`** (or **`GET /v1/catalog`** at build time) |
| **Per-route prerendered HTML** | Build-time prerender step renders static HTML snapshots for each indexable route into **`dist/`** alongside the SPA shell. The existing viewer-request CloudFront Function rewrites clean URLs to these S3 **`index.html`** keys before origin fetch; there is still **no** CloudFront Function / Lambda@Edge bot detection and no dynamic rendering path. | Vite build output plus the indexable route list |
...
| **Allow** (default) | **`/`**, **`/catalog`**, **`/catalog/mst3k`**, **`/catalog/community`**, **`/catalog/riff-material`**, **`/catalog/movie-night`**, **`/watch/`**, **`/how-to-host-a-watchparty`**, **`/terms`**, **`/privacy`** |
...
| **Indexable filter (current)** | **`episodeIsIndexableForSeo`** / **`catalogEntriesIndexableForSeo`** from **`apps/web/src/catalog/catalogSeo.ts`** (**#397**). Sitemap emits one **`<loc>`** per episode passing the filter at **`/watch/{catalogEpisodeId}`**. M28 originally used **`episodeHasYoutubeLink`**; M37 adds host-aware exclusion for Custom-host rows; official Live extends exclusion for **`catalog: live`** (SEO on **`/live/{slug}`** instead). |
| **Static sitemap routes** | **`/`**, **`/catalog`**, **`/catalog/mst3k`**, **`/catalog/community`**, **`/catalog/riff-material`**, **`/catalog/movie-night`**, **`/download`**, **`/how-to-host-a-watchparty`**, **`/terms`**, **`/privacy`** - each as an apex absolute **`<loc>`** (trailing slash on **`/`** only). Hub **`/catalog`** remains; the four subcategory paths and app install instructions page extend the same **`STATIC_INDEXABLE_ROUTES`** list (originally five static routes at M28 ship; current contract is **ten**). **Official Live:** add each enabled **`/live/{slug}`** (first: **`/live/mst3k-forever-a-thon`**) to the static/indexable packaging set when Live ships — count assertions follow the extended list. **`catalog: live`** episodes stay out of **`/watch/{id}`** sitemap entries via **`episodeIsIndexableForSeo`**. |
...
| **Prerender outputs** | Overwrite **`dist/index.html`** with home head tags. Write **`dist/catalog/index.html`**, **`dist/catalog/mst3k/index.html`**, **`dist/catalog/community/index.html`**, **`dist/catalog/riff-material/index.html`**, **`dist/catalog/movie-night/index.html`**, **`dist/download/index.html`**, **`dist/how-to-host-a-watchparty/index.html`**, **`dist/terms/index.html`**, **`dist/privacy/index.html`**, and **`dist/watch/{catalogEpisodeId}/index.html`** for each episode passing **`episodeIsIndexableForSeo`**. Write **`dist/spa-shell.html`** (generic **`noindex`** shell). Subcategory keys follow **`staticRouteDistPath`** from the shared indexable list (one **`index.html`** per static path; originally five static prerender objects at M29 ship; current contract is **ten**). |
...
| **Catalog source / filter** | Same as M28: committed **`data/catalog/episodes.json`**; **`catalogEntriesIndexableForSeo`** (**#397**). Custom-host and no-YouTube-id episodes get **no** prerender object; direct **`/watch/{id}`** requests fall through to **`spa-shell.html`**. |
| **Origin** | **`process.env.VITE_PUBLIC_ORIGIN`** when set at build, else **`https://riffsync.tv`**. |
| **CloudFront clean URLs / fallback** | **`infra/cdk/lib/cloudfront-canonical-redirect.ts`** provides the viewer-request CloudFront Function for both canonical-host redirects and clean SPA URL serving. It rewrites extensionless viewer URIs to prerendered S3 object keys before origin fetch (**`/catalog`** → **`/catalog/index.html`**, **`/watch/{id}`** → **`/watch/{id}/index.html`**, trailing slash → **`index.html`**) and leaves extensioned objects such as **`robots.txt`**, **`sitemap.xml`**, assets, and **`spa-shell.html`** unchanged. **`static-site-stack.ts`** still maps true S3 **403** / **404** misses to **`/spa-shell.html`** (**200**, **ttl 0**) so ephemeral deep links do not serve home canonical metadata from **`index.html`**. No Lambda@Edge is introduced. |
...
| **CI assertion** | Extend **`npm run verify:seo-artifacts`** (or add **`verify:prerender`**) so **`web-app`** asserts: **`spa-shell.html`** exists with **`noindex`**; static indexable paths have **`index.html`** at the S3 keys above; watch prerender count equals **`catalogEntriesIndexableForSeo(entries).length`**; sample files contain expected **`<title>`** / canonical for **`/`** and one known **`/watch/{id}`** fixture episode. |
| **Unit tests** | **`apps/web/src/seo/routeHeadTags.test.ts`** covers all static route shapes in **`STATIC_INDEXABLE_ROUTES`** (**ten** paths in the current contract, including the four catalog subcategory routes and **`/download`**) plus **`/watch/:id`** with and without **`tagline`** / poster art. Exact title/description fixture strings are interface-owned. |
...
### public-site-seo
- No open decisions remain for **`robots.txt`** / **`sitemap.xml`** generation (M28 — #325). See *Decisions (M28 — robots.txt and sitemap.xml — #325)* above. Current static indexable set is **ten** routes (five original + four catalog subcategory paths + **`/download`**); generation shape unchanged.
...
- No open decisions remain for host-aware watch-route indexability (M37 — #397). See *Decisions (M37 — host-aware SEO indexable filter — #397)* above.
...
| Topic | Decision |
| --- | --- |
| **Static list** | Extend **`apps/web/src/seo/indexableRoutes.ts`** → **`STATIC_INDEXABLE_ROUTES`** with **`/catalog/mst3k`**, **`/catalog/community`**, **`/catalog/riff-material`**, **`/catalog/movie-night`** (after hub **`/catalog`**) and **`/download`**. **`staticRouteDistPath`** yields **`catalog/<slug>/index.html`** and **`download/index.html`** for those paths. Sitemap and prerender CI counts follow list length (**10** static + indexable watch episode count per **`catalogEntriesIndexableForSeo`**). |
...
| **Verify / unit tests** | Refresh **`verify:seo-artifacts`** and **`routeHeadTags.test.ts`** (and any fixed-count wording) for **ten** static route shapes. |
| **Production smoke** | **`scripts/launch-readiness/smoke-production.mjs`** checks the clean **`https://riffsync.tv/catalog/mst3k`** URL returns **200**, contains the MST3K prerender **`<title>`** and apex canonical, and does **not** contain **`noindex`**. Invoked via root **`npm run smoke:production`** after SPA publish; **not** PR-CI-wired. |

### Decisions (M37 — host-aware SEO indexable filter — #397)
...
| **Predicate** | **`episodeIsIndexableForSeo`** returns **`false`** when **`readCatalogPlaybackHost(ep) === 'custom'`** (even if **`youtubeVideoId`** is set) or when **`catalog === 'live'`**. Otherwise requires non-empty trimmed **`youtubeVideoId`**. **`embedAllows`** and **`customPlaybackUrl`** do not affect indexability. Official Live SEO uses **`/live/{slug}`** packaging, not **`/watch/{id}`**. |
| **Consumers** | Replace **`catalogEntriesWithYoutubeLink`** in **`generateSeoArtifacts.ts`**, **`generate-seo-artifacts.mjs`**, **`prerender-indexable-routes.mjs`**, and **`verify-seo-artifacts.mjs`**. |
| **`robots.txt`** | Unchanged; Custom-only **`/watch/:id`** has no prerender artifact and falls through to **`spa-shell.html`**. |
| **Tests** | **`catalogSeo.test.ts`** host-aware matrix; extend **`generateSeoArtifacts.test.ts`** with Custom-host fixture asserting no watch **`<loc>`**. |
...
| **Smoke checks (normative)** | (1) **`https://riffsync.tv/`** returns **200**; (2) **`https://www.riffsync.tv/lobby`** returns **301** with **`Location: https://riffsync.tv/lobby`**; (3) **`robots.txt`** and **`sitemap.xml`** return **200**; (4) **`/`** clean URL HTML contains home **`<title>`**, apex canonical, no **`noindex`**, and no **`www.riffsync.tv`** absolute URLs; (5) **`/catalog`** clean URL HTML contains catalog **`<title>`**, apex canonical, and no **`noindex`**; (6) **`/catalog/mst3k`** clean URL HTML contains MST3K **`<title>`**, apex canonical, and no **`noindex`**; (7) **`/watch/101-the-crawling-eye`** clean URL HTML contains episode **`<title>`**, apex canonical, and no **`noindex`**; (8) fetched **`index.html`** body contains no **`www.riffsync.tv`** host strings. Script exits **0** only when all pass. |
