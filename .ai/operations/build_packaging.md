...
| **Static sitemap routes** | **`/`**, **`/catalog`**, **`/catalog/mst3k`**, **`/catalog/community`**, **`/catalog/riff-material`**, **`/download`**, **`/how-to-host-a-watchparty`**, **`/terms`**, **`/privacy`** - each as an apex absolute **`<loc>`** (trailing slash on **`/`** only). Hub **`/catalog`** remains; the three public subcategory paths and app install instructions page extend the same **`STATIC_INDEXABLE_ROUTES`** list. **Official Live:** add **`/live/{id}`** for each **`catalog: live`** row during sitemap/prerender generation. **`catalog: live`** episodes stay out of **`/watch/{id}`** sitemap entries via **`episodeIsIndexableForSeo`**. |
