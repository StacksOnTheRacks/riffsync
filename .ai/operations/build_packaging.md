...
| **Predicate** | **`episodeIsIndexableForSeo`** returns **`false`** when **`readCatalogPlaybackHost(ep) === 'custom'`** (even if **`youtubeVideoId`** is set). Otherwise requires non-empty trimmed **`youtubeVideoId`**. **`embedAllows`** and **`customPlaybackUrl`** do not affect indexability. |
