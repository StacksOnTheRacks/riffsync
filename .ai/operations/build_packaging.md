...
- Extend **`apps/web/src/seo/indexableRoutes.ts`** → **`STATIC_INDEXABLE_ROUTES`** with **`/catalog/mst3k`**, **`/catalog/community`**, **`/catalog/riff-ready`**, **`/catalog/movie-night`**; confirm **`staticRouteDistPath`** yields **`catalog/<slug>/index.html`** for each. Sitemap count and prerender CI assertions follow list length (**9** static + YouTube-linked episode count) — no separate counting scheme.
- Refresh **`verify:seo-artifacts`** / **`routeHeadTags.test.ts`** expectations that hard-code a fixed static count or "six static route shapes" wording when the list grows to nine. Exact title/description/canonical copy strings for the four subcategory routes are **interface-owned** (ops only needs paths present in the shared indexable list).
- Decide whether **`scripts/launch-readiness/smoke-production.mjs`** gains one optional post-deploy check for a subcategory canonical (e.g. **`/catalog/mst3k`**) or stays at today's **`/`** + fixture **`/watch/...`** coverage for this milestone. Not PR-CI-wired either way.
