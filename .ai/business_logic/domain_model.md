...
- **Public discoverable surface:** the subset of routes that represent durable, indexable fan-facing content for search engines and social sharing, distinct from ephemeral, authenticated, or receiver-only surfaces that must stay out of search.
  ...
  | --- | --- |
  | **`/`**, **`/catalog`**, **`/catalog/mst3k`**, **`/catalog/community`**, **`/catalog/riff-material`**, **`/catalog/movie-night`**, **`/download`**, **`/watch/:catalogEpisodeId`** (non-**`live`** YouTube-host rows), **`/live/:slug`** (enabled official Live channels), **`/how-to-host-a-watchparty`**, **`/terms`**, **`/privacy`** | **`/room/:roomId`** (and **`/room/:roomId/experimental/:experimental`**), **`/lobby`**, **`/account`**, **`/admin/*`**, **`/cast/receiver`**, **`/privacy/data-removal`**, **`/auth/callback`**, **`/admin/auth/callback`** |
...
- **`playbackHost` (catalog episode):** **`youtube`** | **`custom`** — **per-catalog-episode** staff setting (not a per-room override at create). **`youtube`** rows use YouTube URL / video id and retain YouTube playable checks at room create. **`custom`** rows use a staff-curated **HTTPS** movie-page URL in a generic iframe; **`youtubeVideoId` is not required** for room create when host is **`custom`**. Official Live source episodes use **`playbackHost: youtube`** with **`catalog: live`**.
- **`catalog` (catalog episode):** **`mst3k`** | **`community`** | **`riff_material`** | **`movie_night`** | **`other`** | **`live`**. Public browse exposes a subset only (**`PUBLIC_CATALOG_CATEGORIES`**); **`other`** and **`live`** are omitted from public catalog surfaces.
- **`roomMode`:** **`theater`** (default on room open) | **`videoChat`** — host-authoritative layout policy; one current value per room; fan-out to all connected participants and late joiners via durable room document + realtime sync. While **`avDisabled`** is false, **Video Chat** remains selectable in the host control bar as a normal A/V room mode, not a **Beta** or **Experimental** mode. **Not used** on official Live surfaces (hostless).

## Invariants

1. **Lawful playback:** app never hosts MST episode files as a communal CDN. **Hosted watch parties:** the room admin uses **official embeds** or **staff-curated lawful HTTPS pages** loaded in RiffSync iframes (**YouTube IFrame API** or **generic iframe** for **Custom** host), and guests receive **browser-mediated realtime media** derived from that viewing surface. **Official Live:** every viewer loads the staff-bound **YouTube live embed** locally (no RiffSync rehost/transcode, no SFU movie relay). Not a separate licensed file library operated by RiffSync.
...
9. **Catalog title:** never replaced by TMDB **`title`** / **`original_title`**. This extends to public search and share metadata — page titles, meta descriptions, and Open Graph/Twitter tags for **`/watch/:catalogEpisodeId`** and **`/live/:slug`** always source the catalog **`title`** field (or LiveChannel SEO override derived from it), never TMDB's **`title`** or **`original_title`**.
10. **Public catalog read** does not require authentication (hub **`/catalog`** and all catalog subcategory routes alike).
11. **Public discoverability boundary:** only the durable public surfaces listed under **Public discoverable surface** - including the catalog hub, its subcategory browse routes, app install instructions, and enabled official **`/live/:slug`** channels - are indexable. Ephemeral per-instance state (**user-created rooms**, **lobby**) and authenticated or receiver-only surfaces (**account**, **admin**, **Cast receiver**, **auth callbacks**) never carry indexable metadata or sitemap entries, regardless of the rendering or build mechanism that produces them.
...
17. **Official Live is hostless:** **`/live/:slug`** has no room-admin capture path and does not require an online **`hostSub`** for playback. Chat uses the bound system **`roomId`** with the same RoomChat TTL and rate limits as watch-party rooms.
18. **Live catalog isolation:** episodes with **`catalog: live`** are staff library sources for LiveChannel bindings only. They must not appear on public catalog browse, home carousel/spotlight, or public create-room tile flows. Primary fan URL is **`/live/:slug`**, not **`/watch/:id`**.
...
| Public catalog SEO indexing scope? | Catalog hub, catalog subcategory browse routes, app install instructions, non-**`live`** YouTube-host episode landings, enabled official **`/live/:slug`** channels, host-help, and legal pages are indexable; user-created rooms, lobby, account, admin, Cast receiver, and auth callbacks stay **`noindex`** - mirrors the existing lawful-playback and identity-mode boundaries, not a new access rule. |
...
| Server vs client playable helpers? | Server: **`validateCatalogRowForRoomSeed`** in **`catalog-room-playback-gate.ts`**. Client browse: **`episodeIsPlayableInApp`** / **`catalogEntriesPlayableInApp`** in **`catalogPlayback.ts`** (**#396**). SEO indexable: **`episodeIsIndexableForSeo`** / **`catalogEntriesIndexableForSeo`** in **`catalogSeo.ts`** (**#397**). |
| SEO indexable predicate (#397)? | **`false`** when **`playbackHost` is `custom`** or **`catalog` is `live`**. Otherwise **`true`** only with non-empty trimmed **`youtubeVideoId`**. Optional YouTube metadata on Custom rows does **not** restore indexability. Official Live SEO is owned by **`/live/:slug`**, not **`/watch/:id`**. |
...
## Decisions (answered — official live channels)
