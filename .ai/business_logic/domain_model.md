...
- **Public discoverable surface:** the subset of routes that represent durable, indexable fan-facing content for search engines and social sharing, distinct from ephemeral, authenticated, or receiver-only surfaces that must stay out of search.
  ...
  Ephemeral, authenticated, and receiver-only routes carry no durable identity worth surfacing to crawlers — this mirrors the existing **Identity modes** boundary below, not a new access rule. **`/watch/:catalogEpisodeId`** is indexable only for **YouTube-host** episodes with a non-empty trimmed **`youtubeVideoId`** (**`episodeIsIndexableForSeo`**). **Custom-host** rows are **never** indexable (including when optional YouTube enrichment exists). Rows without a YouTube video id have no indexable surface under today's SEO packaging and are **excluded from indexing and the sitemap**. **Custom playback** does not by itself add `/watch/:id` to the sitemap.
...
| SEO indexable predicate (#397)? | **`false`** when **`playbackHost` is `custom`**. Otherwise **`true`** only with non-empty trimmed **`youtubeVideoId`**. Optional YouTube metadata on Custom rows does **not** restore indexability. |
