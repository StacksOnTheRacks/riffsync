...
- **Official Live** adds catalog enum value **`live`** and a seeded **LiveChannel** registry (**`slug`**, **`catalogEpisodeId`**, system **`roomId`**, **`enabled`**) documented in **`data_model.md`**. System Live rooms reuse RoomChat / RoomPresence without host-capture authority.
