...
| **DM process class** | DM history sync-on-open and realtime push stay Lambda / API Gateway class (write-then-fan-out analogue). **HTTP POST send** + **Fan DM WebSocket** push on **`FanConnections`** (#360). Account-lifetime retention may later add **Scheduler → Lambda** purge on account closure; that reuses the existing scheduled-job class, not a new process topology. |
| **Fan DM session** | Client opens **Fan DM WebSocket** when signed-in fan loads friends/DM surfaces (main-site or room Friends pane). Independent lifecycle from **`RoomRealtimeSdk`** / **`ChatSession`**. |
