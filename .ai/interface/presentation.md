...
| **Catalog browse IA** | Public **`/catalog`** is a **hub**: four large **text** entry links (no imagery) in the page-header subtitle slot above title-search / sort and the retained mixed/all-titles title grid. Link order and labels: **MST3K**, **Community**, **Riff Material**, **Movie Night** (display names only). **Era chips are removed** from **`/catalog`**. Public copy uses **Riff Material**; route slug **`riff-material`**. Staff-only **`other`** and official Live source **`live`** never appear on the hub, nav dropdown, or subcategory chrome. Official Live discovery lives on **`/lobby`**. |
...
| **`/catalog`** | **`RiffSync Catalog - Browse the Library`** | **`Browse RiffSync episodes across MST3K, Community, Riff Material, and Movie Night. Pick a title and start a lawful YouTube watch party. Unofficial fan project.`** | **`{origin}/catalog`** | **`{origin}/og-card.png`** |
...
| **`/live/:slug`** | **`{episode.title} - Live on RiffSync`** where **`slug`** is a **`catalog: live`** episode id | With episode **`tagline`**: **`{tagline}`**. Without **`tagline`**: **`Watch {episode.title} live on RiffSync with room chat.`** | **`{origin}/live/{id}`** | Absolute bound-episode **`posterImageUrl`**, else **`backdropImageUrl`**, else **`{origin}/og-card.png`** |
...
Meta titles/descriptions/OG for **`/watch/:id`** and **`/live/:slug`** always use the catalog **`title`** field (or LiveChannel SEO override derived from it), never TMDB's **`title`**/**`original_title`** (**`business_logic/domain_model.md`** Invariant 9). **`catalog: live`** rows are not packaged as indexable **`/watch/:id`** landings.
...
- **Nav chrome:** Main-nav **Catalog** parent navigates to **`/catalog`**; a dropdown lists the same four subcategory destinations in the same order and display names as the hub entry links (no helper microcopy). **`other`** and **`live`** are omitted from hub links, dropdown, and subcategory chrome. Official Live entries are discovered from **`/lobby`**.

### Official Live presentation
...
| **Party capture** | URL remains **`{origin}/watch/{catalogEpisodeId}?partyCapture=1`**; inner player swaps to generic iframe for Custom. Capture tab is **chrome-free** (no media picker / capture banner). Share-quality lives on the host **Room** sidebar tab. **Open Source Tab** reuses one named browsing context (`riffsync-host-source`). Host shares this RiffSync tab via browser picker + WebRTC. |
...
| **Custom-host** | Uses the same capture workflow as YouTube: **Open Source Tab** opens the RiffSync party-capture watch tab (**`/watch/:id?partyCapture=1`**), then **Share Source Tab** publishes that tab over WebRTC. Do not render **`SoloCustomIframePlayer`** inside the room host shell. |
| **YouTube-host** | Unchanged capture workflow: external YouTube tab when not embeddable; RiffSync party-capture watch tab when embeddable. Existing host capture preview **`<video>`** when **`captureStream`** is active. |
| **Capture vs controls** | When host **`captureStream`** is **inactive**, player shell shows share controls and intro copy. When **`captureStream`** is **active**, player shell shows capture preview **`<video>`**. |
| **Source tab ownership** | Party-capture tab (**`/watch/:id?partyCapture=1`**) owns the Custom iframe instance through **`SoloCustomIframePlayer`**. The room page does not mount a second Custom iframe. |
| **React ownership** | **`RoomPlaybackPanel`** owns share controls and WebRTC preview only. **`TheaterPlayback.setYoutubeMountElement`** / dataset metadata is **not** the Custom presentation path; WebRTC video binding stays in **`TheaterPlayback`**. |
| **Episode retarget** | When host **`PATCH`** changes **`catalogEpisodeId`**, source-tab resolution follows the refreshed room/catalog episode state; no in-room Custom iframe is remounted. |
...
- Two controls (**Camera**, **Microphone**) sit at the **top of the sidebar** (above the Chat / People / Friends / Room / Profile tabs) when the viewer has a **fan JWT** (signed-in fan or host using participant A/V). Cast and Link TV share that same top A/V strip.
...
| **TV layout constraints** | The receiver root fills **100vw x 100vh** on a dark, chrome-free background. The stage primary occupies the full available 16:9 TV canvas using contained media/object sizing so it is not cropped on **1280x720**, **1920x1080**, or **3840x2160** displays. The chat overlay is anchored bottom-right inside a TV-safe inset of at least **24px** at 720p, **32px** at 1080p, and **64px** at 4K. Its width is **clamp(22rem, 34vw, 42rem)** and never exceeds **40%** of the stage width. Its height fills the stage between the matching TV-safe top and bottom insets (nearly full stage height). The overlay has no full-panel glass background; each chat line uses its own translucent glass chip, right-aligned text at a TV-legible size, a hidden scrollbar, and fades each line out about **10 seconds** after first sight on the TV client. Messages bottom-pack inside that tall column. The overlay scrolls internally when needed and stays anchored on the right so it does not cover the center of the stage-primary video. |
...
| **Receiver placeholder copy** | Before the first sender presentation snapshot, the receiver shows **`Waiting for party presentation...`** in a polite status surface. When stage-primary playback is not yet available, the stage placeholder uses **`Waiting for room video...`**. When receiver playback is blocked by provider or autoplay policy, the stage placeholder uses **`Playback needs attention on the sender.`** and does not expose provider error codes, receiver device names, or participant identifiers. Empty chat overlay state renders no placeholder copy. |
...
| **AV toggles** | Participant camera/mic toggles remain visible at the top of the sidebar on **Friends** tab when fan JWT present. |
...
## Decisions (M37 — host source-tab Custom flow — #394)

| Topic | Decision |
| --- | --- |
| **Component reuse** | **`SoloCustomIframePlayer`** from **#393** is reused by solo watch and the party-capture watch tab only. It is not rendered inside **`RoomPlaybackPanel`**. |
| **Mount strategy** | One Custom iframe instance lives in the opened source tab. The room page keeps share controls before capture and capture preview after capture starts. |
| **Visibility rule** | Share controls when host **`!captureStream`**; capture preview **`<video>`** when **`captureStream`** active. |
| **Playback mirrors** | Keep **`room.playbackHost`** and **`room.customPlaybackUrl`** from **`GET /v1/rooms`** (**#392**) as durable room playback metadata and diff inputs, not as room iframe mount inputs. |
...
| **TheaterPlayback scope** | **No Custom iframe wiring through **`TheaterPlayback`**. Keep **`setYoutubeVideoIdForTheater`** for YouTube-host episode retarget only; Custom playback stays in the party-capture watch tab. |
