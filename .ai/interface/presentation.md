...
| **`/live/:slug`** | **`{channelOrEpisodeTitle} - Live on RiffSync`** (LiveChannel SEO override title when set; else bound episode **`title`**) | With episode **`tagline`**: **`{tagline} — watch live on RiffSync with room chat. Unofficial fan project.`** Without **`tagline`**: **`Watch {channelOrEpisodeTitle} live on RiffSync with room chat. Unofficial fan project.`** | **`{origin}/live/{slug}`** | Absolute bound-episode **`posterImageUrl`**, else **`backdropImageUrl`**, else **`{origin}/og-card.png`** |
