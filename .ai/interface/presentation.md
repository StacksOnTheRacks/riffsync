...
| **Party capture** | URL remains **`{origin}/watch/{catalogEpisodeId}?partyCapture=1`**; inner player swaps to generic iframe for Custom. Capture tab is **chrome-free** (no media picker / capture banner). Compact title select lives on the room host **`RoomPlaybackPanel`**; share-quality lives on the host **Room** sidebar tab. **Open Source Tab** / title switch reuse one named browsing context (`riffsync-host-source`). Host shares this RiffSync tab via browser picker + WebRTC. |
