...
1. **Availability gate:** In normal room view, Cast-capable senders see icon-only **Cast** in the A/V strip when sender support is detected. **Link TV** is always offered for pairing-code TVs. Unsupported Cast never blocks Link TV, the normal stage, chat, expanded view, host controls, participant A/V, or room controls.
...
2. **Available:** Enable **Cast** in the A/V strip when the SDK reports availability and **`CastContext.setOptions`** succeeds with **`VITE_CAST_RECEIVER_APP_ID`** and **`ORIGIN_SCOPED`** auto-join policy. Always offer **Link TV**.
3. **Checking / unavailable:** While checking, omit Cast. When unavailable because the SDK is absent, blocked, timed out, missing **`VITE_CAST_RECEIVER_APP_ID`**, or cannot configure **`CastContext`**, disable Cast and keep Link TV; keep any local Cast status at the A/V Cast surface only. Do not use chat drawer, video-relay status, host feedback, room error boundaries, or global announcer copy for availability failure.
