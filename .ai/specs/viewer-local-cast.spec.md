...
Cast availability is shown only in normal room view after local sender support and public receiver configuration are present. Until the Cast path is repaired and release-ready, the sender start entry point is also gated by the existing experimental room feature opt-in. This opt-in gates Cast only. Unsupported, blocked, unconfigured, or non-experimental Cast omits the start action or shows local recoverable status without blocking room snapshot, chat, SFU media, participant A/V, expanded view, host controls, or normal playback.
