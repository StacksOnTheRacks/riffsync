...
| --- | --- | --- |
| **Anonymous guest** | Opaque **`sessionId`** (UUID) + **display name** in **`localStorage`** once the user crosses **lobby**, **joins `/room/:id`**, or **joins `/live/:slug`** (**lazy mint**); **`X-Session-Id`** + WS **`$connect`**. | **Browse**, **join**, **watch**, **view room chat** (text, GIFs, reactions, avatars)—including official Live channels—**cannot** **send** chat, **react**, upload avatars, create rooms, **publish** WebRTC (host screen share **or** participant camera/mic), or use **friends / DM** manage or send. May **subscribe** to participant A/V when the room **`avDisabled`** kill switch is off (watch-party rooms only; Live v1 has no participant A/V). |
