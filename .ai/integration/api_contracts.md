...
| **`POST /v1/rooms` (create)** | **Fan Cognito JWT** — **`hostSub`** on new room **= JWT `sub`**. **`sessionId`** optional telemetry only—**not** host binding. | Mint **`roomId`**, seed **`catalogEpisodeId`** / visibility / **`playbackExpectation`** per payload. Lookup Catalog row; resolve **`playbackHost`** (missing/invalid → **`youtube`**). **YouTube-host:** require non-empty **`youtubeVideoId`** matching **`/^[a-zA-Z0-9_-]{11}$/`** — **no** live YouTube API probe; **`embedAllows`** is **not** evaluated (external-tab party path remains valid). **Custom-host:** require non-empty trimmed **`customPlaybackUrl`** with **`https://`** prefix; **do not** require **`youtubeVideoId`**. Denormalize **`playbackHost`**, **`customPlaybackUrl`**, and optional **`youtubeVideoId`** onto the **Rooms** item. Server sets **`roomMode: theater`** and **`avDisabled: false`**; **`201`** echoes playback fields plus AV defaults. |
...
| **`GET /v1/lobby`**, lobby joins | **`sessionId`** via **`X-Session-Id`** (guest); optional JWT if viewer signs in. | Lobby rows for discovery/join validation — **no** **`roomMode`** or **`avDisabled`** on listing payload (#101). Official Live system rooms are **not** ordinary lobby parties. |
| **`GET /v1/live`** | None required for read. | List official Live channels from **`catalog: live`** rows. Returns **`version: 1`** and **`channels[]`** with **`slug`** (catalog id), **`path`**, **`catalogEpisodeId`**, derived system **`roomId`**, playback fields, and display/SEO fields. |
| **`GET /v1/live/{slug}`** | None required for read; optional **`X-Session-Id`**. | Resolve a **`catalog: live`** row by **`id === slug`**. Returns **`slug`**, **`path`**, **`catalogEpisodeId`**, playback fields needed for YouTube embed, derived system **`roomId`**, and SEO/display title. Ensures the system room exists/refreshed for chat. **404** when unknown or when the row is not **`catalog: live`**. |
| **`PATCH /v1/rooms/{roomId}`** | **Fan JWT**; **`JWT.sub === room.hostSub`**. | **Mutable current episode**, advisory labels, visibility flags—conditional **`version`** writes (**`409`** on stale **`version`**). When **`catalogEpisodeId`** changes, re-run the same Catalog playback gate as **`POST /v1/rooms`** and refresh denormalized **`playbackHost`**, **`customPlaybackUrl`**, and **`youtubeVideoId`** on the room item. Same host-only gate for durable **`roomMode`** (**`theater`** \| **`videoChat`**) and **`avDisabled`** (boolean kill switch). Partial body: omit keys to leave fields unchanged; **`roomMode`** / **`avDisabled`** reject **`null`** (**`400`**). **`broadcastCaptureActive`** retains existing boolean or **`null`**-clear semantics. One request may atomically update **`roomMode`**, **`avDisabled`**, and **`broadcastCaptureActive`** (#109). **`200`** echoes playback fields, **`roomMode`**, **`avDisabled`**, **`broadcastCaptureActive`**, and bumped **`version`**. |
...
| **`GET/POST /v1/cast/diag`** | None (public, throttled). | Allowlisted Cast launch hop events for ops. **`GET ?e=`** is the receiver HTML pixel. **`POST`** JSON: **`event`**, **`hop`**, optional UUID **`attemptId`**, sanitized **`code`** / **`description`** / **`details`**. Emits **`RiffSync/Cast`** EMF plus a structured log. Drops unknown fields. |
...
| **`POST`**, **`PATCH`**, **`DELETE /v1/admin/catalog/episodes/:id`** | Catalog CRUD; payload compatible with **`data/catalog/catalog.schema.json`** (+ Dynamo-only fields per **`docs/architecture.catalog-images.md`**). Accepts **`playbackHost`**, **`customPlaybackUrl`**, **`catalog`** including **`live`**, and existing YouTube fields; host-conditional validation at save. **`catalog: live`** rows publish Live channel sources (not public browse titles). No separate LiveChannel registry CRUD under **`/v1/admin/*`** in v1. |
...
- **Viewer-local Cast / TV link:** Cast unavailable, start failure, active receiver disconnect, receiver playback blocked, stop failure, and cleaned-up state are client-only sender lifecycle states. They are not room snapshot fields, lobby fields, WebSocket inbound routes, WebSocket fan-out payloads, **`share_state`** variants, SFU token claims, or room diagnostics drawer fields. Short-lived TV pairing codes use **`POST/GET/PUT /v1/tv/pairing*`** (including **`.../release`**) only (Dynamo TTL); they must not mutate room authority. Cast launch hop telemetry uses public **`GET/POST /v1/cast/diag`** (allowlisted events only; see **`operations/observability.md`**). It must not accept room ids, device names, or tokens.
...
| **History success body** | **`200`** **`{ "messages": [ { "messageId", "senderSub", "kind", "body", "sentAt", ...gifFields } ], "nextCursor": string \| null }`**. Messages ordered **newest-first** in the array. **`kind`** is **`text`** or **`gif`**; gif rows include **`giphyId`**, **`renditionUrl`**, and optional **`title`**, **`width`**, **`height`**. When **DmThread** has prior **`closedAt`** from unfriend (#362), exclude rows with **`sentAt <= closedAt`** from all pages (retained in storage, not returned). |
...
| Route | Auth | Contract |
| --- | --- | --- |
| **`POST /v1/dm/threads/{pairKey}/messages`** | Fan JWT | Send **DirectMessage** when **Friendship** active and **DmThread** **`open`**. Text request body: **`{ "messageId": "<uuid>", "kind": "text", "body": "<string>" }`** with trimmed non-empty **`body`**, max **2000** chars. GIF request body: **`{ "messageId": "<uuid>", "kind": "gif", "giphyId", "renditionUrl", "title"?, "width"?, "height"? }`**. **201** echoes persisted fields. After persist, **`PostToConnection`** **`dm_message`** push to peer's **`FanConnections`** (best-effort). Throttle **20/min** per caller **`fanSub`**. |
...
| Chromecast room API? | **No room/WS fields.** Cast channel remains sender-local. TV browser pairing uses dedicated **`/v1/tv/pairing`** HTTP (create/poll/claim/presentation/release) with TTL codes; never room snapshot, lobby, `share_state`, or SFU claim changes. Host **Stop Link TV** calls **`POST /v1/tv/pairing/{pairingId}/release`**; TV poll observes **`released`** and tears down SFU playback. |
...
| Room snapshot denormalization? | **Yes** — **Rooms** item stores **`playbackHost`**, **`customPlaybackUrl`**, and optional **`youtubeVideoId`** at create and on episode-change **`PATCH`**; echoed on **`GET`**, **`201`**, and **`PATCH` `200`**. |
| **`embedAllows` at room gate?** | **No** — room create/patch does **not** reject YouTube-host rows with **`embedAllows: false`**; client Start Party / in-app embed gating remains separate (#396). |
| Shared gate helper? | **`catalog-room-playback-gate.ts`** (or equivalent under **`infra/cdk/lambda/`**) exports **`readCatalogPlaybackHost`** and **`validateCatalogRowForRoomSeed`**; used by **`room-create`** and **`room-patch`**. |
| Episode picker lobby metadata? | **Durable room write only** — no new WebSocket envelope; **`GET /v1/lobby`** reads updated **`catalogEpisodeId`**, **`displayTitle`**, **`playbackHost`**, and optional **`youtubeVideoId`** on next poll. |

### Room create / patch catalog gate deny codes (#392)

Applies to **`POST /v1/rooms`** and host **`PATCH /v1/rooms/{roomId}`** when **`catalogEpisodeId`** is seeded or changed. Body includes **`code`** (stable) and **`error`** (human message).

| **`code`** | HTTP | When |
| --- | --- | --- |
| **`catalog_episode_not_found`** | **404** | Catalog **`GetItem`** returned no row for **`catalogEpisodeId`**. |
| **`catalog_episode_youtube_id_missing`** | **400** | Resolved host **`youtube`**, but **`youtubeVideoId`** is missing, empty, or fails **`/^[a-zA-Z0-9_-]{11}$/`**. |
| **`catalog_episode_custom_url_missing`** | **400** | Resolved host **`custom`**, but **`customPlaybackUrl`** is missing, empty, or does not start with **`https://`**. |
