...
| Chromecast room API? | **No room/WS fields.** Cast channel remains sender-local. TV browser pairing uses dedicated **`/v1/tv/pairing`** HTTP (create/poll/claim/presentation) with TTL codes; never room snapshot, lobby, `share_state`, or SFU claim changes. |
