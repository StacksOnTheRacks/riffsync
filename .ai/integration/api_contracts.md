...
| **`GET /v1/live/{slug}`** *(planned)* | None required for read; optional **`X-Session-Id`**. | Resolve seeded/enabled **LiveChannel** by slug: return **`slug`**, **`catalogEpisodeId`**, bound episode playback fields needed for YouTube embed, system **`roomId`**, and SEO/display title. **404** when unknown/disabled. v1 may equivalently ship client-seeded registry + existing catalog/room reads if this route is deferred — behavior must match **`official-live-channels`**. |
...
| **`POST`**, **`PATCH`**, **`DELETE /v1/admin/catalog/episodes/:id`** | Catalog CRUD; payload compatible with **`data/catalog/catalog.schema.json`** (+ Dynamo-only fields per **`docs/architecture.catalog-images.md`**). Accepts **`playbackHost`**, **`customPlaybackUrl`**, **`catalog`** including **`live`**, and existing YouTube fields; host-conditional validation at save. **`catalog: live`** rows are Live channel sources (not public browse titles). No LiveChannel registry CRUD under **`/v1/admin/*`** in v1 (seeded config). |
