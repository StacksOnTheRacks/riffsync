...
| Use | Mechanism | Contract |
| --- | --- | --- |
| **Playback (Custom-host episodes)** | **Generic HTTPS iframe** on **`/watch/:catalogEpisodeId`** and party-capture (**`?partyCapture=1`**) when catalog **`playbackHost` is `custom`**. The room host shell shows source-tab share controls instead of embedding Custom directly. | Staff enter **known embeddable** HTTPS movie-page URLs (**HTTPS only**, any domain, no domain allowlist at validation). RiffSync does **not** rehost or transcode. **No YouTube IFrame API sync** for Custom URLs. Iframe embed failure (X-Frame-Options): honest error UI; no product runtime fallback beyond staff policy. |
