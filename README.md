...
- **Optional tweaks** — Let guests reroll anonymous display name or lightly edit label if you want nicer social UX.
- **Hosted Cognito account** — **Required to host** (create a room, publish WebRTC, mutate authoritative playback metadata): users **sign up and sign in** through **Amazon Cognito Hosted UI** (email + password on the fan user pool). **Optional for guests:** anonymous **`sessionId`** + display name remains enough to **browse**, **join**, **watch**, and **chat** in someone else’s room. Signed-in hosts get a stable Cognito **`sub`** stored on the room as **`hostSub`** (see **`docs/architecture.server.md`**). **JWT** authorizers gate **`POST /v1/rooms`**, room-admin **`PATCH`**, and publisher/signaling paths as implemented (see **`docs/architecture.frontend.md`**, **`authorization.md`**).
...
- [AWS CDK & GitHub Actions deploy (`infra/cdk/`) — synth, `cfn-lint`, prod workflow](infra/cdk/README.md)
- [Giphy API key, attribution, and operator smoke test (`docs/operations/giphy.md`)](docs/operations/giphy.md)
- [Staff operator onboarding — manual Cognito invites (`docs/operations/operator-onboarding.md`)](docs/operations/operator-onboarding.md)
...
- [Host extension — unpacked install + Room-tab media helper (`apps/host-extension/`)](apps/host-extension/README.md)
