...
import { fetchLiveChannel, type LiveChannelSnapshot } from '../api/liveApi'
...
export function LiveChannelPage() {
  ...
  const [channel, setChannel] = useState<LiveChannelSnapshot | null>(null)
  ...
  useEffect(() => {
    ...
    setLoadError(null)
    setChannel(null)
    if (!slug || !seed?.enabled) {
      setLoading(false)
      setLoadError('Live channel not found')
      return
    }
    void fetchLiveChannel(slug)
      .then((snap) => {
        if (!cancelled) {
          setChannel(snap)
          setLoading(false)
      ...
      .catch((err: unknown) => {
        if (!cancelled) {
          setLoadError(err instanceof Error ? err.message : 'Live channel unavailable')
          setLoading(false)
        }
      })
    return () => {
  ...
  const chat = useLiveChannelChat({
    ...
    displayName,
    fanToken,
    enabled: Boolean(channel?.roomId),
  })

  const { logRef: chatLogRef, showJumpToLatest, jumpToLatestLabel, jumpToLatest } =
    useChatLogStickToBottom(chat.chat.length, true, channel?.roomId ?? 'live')
                ...
                {!fanToken ? (
                  ...
                  >
                    <button
                      ...
                      onClick={() =>
                        void startFanHostedUiSignIn(`/live/${encodeURIComponent(slug)}`).catch(
                          console.error,
                        )
                      }
                    >
